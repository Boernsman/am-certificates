...
def fetch_codes_from_backend(base_url, count, username, password):
    ...
    try:
        response = requests.get(f"{base_url}/erstelle", headers=headers, params={"count": count})
...
def load_from_csv(codes, output_file):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Get certificate codes from csv file and delete those from backend.')
    parser.add_argument('-u', '--url', type=str,
                        default='https://zertifikat.austromagnum.at', help='Base URL of the backend')
    parser.add_argument('-i', '--input', type=str, default='codes.csv', help='Output CSV file (default: codes.csv)')
    parser.add_argument('-c', '--config', type=str, default='../tests/config.ini', help='Authentication credential file')
    
    args = parser.parse_args()
    
    username, password = load_credentials(args.config)
    data = load_from_csv(args.url, args.count, username, password)
